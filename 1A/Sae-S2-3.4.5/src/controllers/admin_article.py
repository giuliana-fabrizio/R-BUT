...
def show_meuble():
    ...
    sql = '''select id_meuble,nom,prix,dateFabrication,id_MeubleType,
    ...
    sql = '''select id_ColoreMeuble, nbStock, libelle_couleur, libelle_materiaux
        from colore 
        inner join couleur on id_couleur = id_ColoreCouleur
        inner join materiaux on id_materiaux = id_ColoreMateriaux
        ;'''
    mycursor.execute(sql)
...
def valid_add_meuble():
    ...
    tuple_insert = (nom, prix, date_fabrication, type_meuble_id, image)
    sql = '''insert into meubles(nom,prix,dateFabrication,id_MeubleType,
             image)
             values(%s,%s,%s,%s,%s);'''
    ...
    tuple_insert = (id_meuble['last_insert_id'], couleur, materiaux, stock)
    sql = '''insert into colore(id_ColoreMeuble, id_ColoreCouleur, id_ColoreMateriaux, nbStock) values(%s,%s,%s,%s);'''
    mycursor.execute(sql, tuple_insert)
    get_db().commit()
...
def edit_meuble(id):
    ...
    mycursor.execute(sql)
    type_meuble = mycursor.fetchall()
    return render_template('admin/article/edit_article.html', meuble=meuble,
...
def valid_edit_meuble():
    ...
    tuple_edit = (nom, prix, date_fabrication, type_meuble_id, image, id)
    ...
    flash('meuble modifié , nom: ' + nom + ' - prix: ' + prix + ' - dateFabrication: ' + date_fabrication +
          ' - type_meuble: ' + type_meuble_id + ' - image: ' +
          image)
    return redirect(url_for('admin_article.show_meuble'))
...
def stock_meuble():
    ...
    mycursor.execute(sql)
    materiaux = mycursor.fetchall()
    return render_template('admin/article/stock_article.html', meuble=meuble, couleur=couleur, materiaux=materiaux)
...
def valid_stock_meuble():
    ...
    stock = request.form.get('stock')
    tuple = (stock, couleur, materiaux, id_meuble)

    sql = '''select * from colore where id_ColoreCouleur = %s and id_ColoreMateriaux = %s and id_ColoreMeuble = %s;'''
    mycursor.execute(sql, (couleur, materiaux, id_meuble))
    colore = mycursor.fetchall()

    if colore == ():
        sql = "insert into colore(nbStock, id_ColoreCouleur, id_ColoreMateriaux, id_ColoreMeuble) values(%s,%s,%s,%s);"
        mycursor.execute(sql, tuple)
    else:
        sql = "update colore set nbStock=nbStock+%s where id_ColoreCouleur = %s and id_ColoreMateriaux =%s and id_ColoreMeuble = %s;"
        mycursor.execute(sql, tuple)
    get_db().commit()
    return redirect(url_for('admin_article.show_meuble'))
